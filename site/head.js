...
ge.start = () => {
	ge.c = document.querySelector('canvas');
...
ge.loop = () => {
	...
	document.querySelector('#fps').innerText = "fps: "+Math.round(1/dt)+" / "+FPS
...
window.addEventListener('keydown', function(event){
	...
	if(!(((event.key == 'I' || event.key == '`' || event.key == '~') && event.ctrlKey) || event.key == 'F12'))
...
// vec0 = new V(0,0);
Object.defineProperty(window, 'vec0', {
  get: function() {
    return new V(0,0);
  },
  configurable: true,
